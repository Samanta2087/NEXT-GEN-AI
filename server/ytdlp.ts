...
export async function analyzeUrl(url: string): Promise<MediaInfo> {
  ...
  return new Promise((resolve, reject) => {
    // Realistic Chrome user agents for spoofing
    const userAgents = [
    ...
    const args = [
      ...
      // Add referer to look like coming from YouTube
      "--referer", "https://www.youtube.com/",
      // Additional headers to appear more like a real browser
      "--add-header", "Accept-Language:en-US,en;q=0.9",
      "--add-header", "Accept:text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",
      // Extractor args for YouTube - use android client which has fewer restrictions
